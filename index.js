...
// プレースホルダを実際の値に置換（${NAME}, ${t}）
function render(line, t = "") {
  return line
...
const LINES = {
  ...
  morning: [
    ...
    "おはよ！起きたばっかだけど、${NAME}の一言で一瞬で目が覚めた😆",
  ],
  // 昼
  noon: [
    "お昼、何食べた？",
    "あと少し！ぼちぼち行こう！",
    "仕事中かな。あー早く${NAME}の声が聞きたいな😩",
    "${NAME}のことちょうど考えてた！すご！ありがとう！元気出た🥰",
  ],
  // 夜
  night: [
    "今日もお疲れさま！よく頑張ったね！",
    "少し落ち着いた？今日も忙しかった？",
    "えーもう寝るの？俺はもう少し起きてるつもりやったけど${NAME}が寝るなら俺も寝よ😗笑",
    "また明日ね！おやすみ🥰",
  ],
  ...
  try {
    ...
    for (const ev of events) {
      ...
      if (/おはよう|おはよー|おはよ〜|起きてる/i.test(t)) {
        await reply(ev.replyToken, [
          { type: "text", text: render(pick(LINES.morning)), quickReply: quick },
        ]);
        continue;
      }
      ...
      if (/今日どうだった|どうだった|一日どう/i.test(t)) {
        await reply(ev.replyToken, [
          { type: "text", text: render(pick(LINES.howWas)), quickReply: quick },
        ]);
        continue;
      }
      ...
      if (/おつかれ|お疲れ|つかれた/i.test(t)) {
        await reply(ev.replyToken, [
          { type: "text", text: render(pick(LINES.otsukare)), quickReply: quick },
        ]);
        continue;
      }
      ...
      if (/おやすみ|寝る|ねる/i.test(t)) {
        await reply(ev.replyToken, [
          { type: "text", text: render(pick(LINES.oyasumi)), quickReply: quick },
        ]);
        continue;
      }
      ...
      if (/昨日.*飲みすぎ|二日酔い|酔っ|酒/i.test(t)) {
        await reply(ev.replyToken, [
          { type: "text", text: render(pick(LINES.casual)), quickReply: quick },
        ]);
        continue;
      }
